...
void Object::_SetCreateBits(UpdateMask* updateMask, Player* target) const
{
    ...
    for (uint16 index = 0; index < valCount; ++index, ++value)
        if (_fieldNotifyFlags & flags[index] || (flags[index] & UF_FLAG_PARTY_MEMBER && hasUnitAll) || (*value && IsUpdateFieldVisible(flags[index], isSelf, isOwner, isItemOwner, isPartyMember)))
            updateMask->SetBit(index);
}
